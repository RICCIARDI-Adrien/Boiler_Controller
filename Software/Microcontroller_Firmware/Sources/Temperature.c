...
signed char TemperatureGetCelsiusValue(TTemperatureID Temperature_ID)
{
	...
	switch (Temperature_ID)
	{
		case TEMPERATURE_ID_OUTSIDE:
			Temperature = ADCGetLastSampledValue(ADC_CHANNEL_ID_EXTERNAL_THERMISTOR);
		...
		case TEMPERATURE_ID_BURNER_OUTPUT_WATER:
			Temperature = ADCGetLastSampledValue(ADC_CHANNEL_ID_INTERNAL_THERMISTOR);
			// Use a straight line representation to determine the Celsius temperature
		...
		case TEMPERATURE_ID_BURNER_INPUT_WATER:
